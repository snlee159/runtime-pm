...
// Minimal task data for dependency selection
              ...
              <>
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onUpdate(subtask.id, {
                        energy_cost: e.target.value as EnergyCost,
                      })
                    }
                    className="px-2 py-1 bg-zinc-900 border border-zinc-800 rounded text-xs"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onUpdate(subtask.id, {
                        focus_depth: e.target.value as FocusDepth,
                      })
                    }
                    className="px-2 py-1 bg-zinc-900 border border-zinc-800 rounded text-xs"
                  >
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onUpdate(subtask.id, {
                        context_type: e.target.value as ContextType,
                      })
                    }
                    className="px-2 py-1 bg-zinc-900 border border-zinc-800 rounded text-xs"
                  >
...
export default function CreateTaskPage() {
  ...
  const [existingTasks, setExistingTasks] = useState<TaskForDependency[]>([]);
        ...
        const generateSubtaskTree = (
          ...
          return tasks.map((st: any, index: number) => {
            ...
            if (st.subtasks && st.subtasks.length > 0) {
              subtask.subtasks = generateSubtaskTree(
                st.subtasks,
                `${parentIndex}${index}-`
              );
            }
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        const insertSubtasksRecursively = async (
          ...
          if (createdSubtasks) {
            for (let i = 0; i < tasks.length; i++) {
              ...
              if (
                tasks[i].depends_on_indices &&
                tasks[i].depends_on_indices!.length > 0
              ) {
      ...
      if (parentDependencyIds.length > 0 && parentTask) {
        ...
        if (parentDepsError) {
          console.error(
            "Failed to create parent task dependencies:",
            parentDepsError
          );
          // Don't throw - dependencies are nice to have but not critical
        }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-zinc-400">
          AI will help break down and estimate your task
        </p>
      </div>
              ...
              >
                ...
                {parentDependencyIds.length > 0 && (
                  <span className="ml-auto px-2 py-0.5 bg-amber-900/50 text-amber-300 rounded text-xs font-semibold">
                    {parentDependencyIds.length} dependency
                    {parentDependencyIds.length !== 1 ? "ies" : ""}
                  </span>
                )}
              </button>
                  ...
                  {existingTasks.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {existingTasks.map((task) => {
                        const isSelected = parentDependencyIds.includes(
                          task.id
                        );
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setParentDependencyIds([
                                    ...parentDependencyIds,
                                    task.id,
                                  ]);
                                } else {
                                  setParentDependencyIds(
                                    parentDependencyIds.filter(
                                      (id) => id !== task.id
                                    )
                                  );
                                }
