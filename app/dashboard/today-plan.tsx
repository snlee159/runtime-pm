...
export function TodayPlan({ plan, tasks, today }: TodayPlanProps) {
  ...
  // Fetch daily inspiration on mount (using local date from props)
  useEffect(() => {
    const fetchInspiration = async () => {
      try {
        const response = await fetch(`/api/daily-inspiration?date=${today}`, {
          cache: "no-store",
        });
  ...
  const primaryTask = tasks.find(
    (t: Task) => t.id === plan.primary_focus_task_id
  );
  const secondaryTasks = tasks.filter((t: Task) =>
    plan.secondary_task_ids?.includes(t.id)
  );
  const multitaskTasks = tasks.filter((t: Task) =>
    plan.multitask_task_ids?.includes(t.id)
  );
    ...
    if (!error) {
      ...
      if (user) {
        ...
        if (!isCompleted) {
          ...
          await autoUncompleteParentTasks(supabase, taskId, user.id);
        }
      }
    ...
    <>
      <div className="max-w-4xl mx-auto">
        ...
        {(() => {
          ...
          const completedCount = allPlanTasks.filter(
            (t) => t.status === "complete"
          ).length;
          ...
          return (
            <div className="mb-8 p-6 bg-gradient-to-r from-purple-900/20 to-blue-900/20 border border-purple-800/50 rounded-lg">
              ...
              <p className="text-sm text-zinc-400">
                ...
                {totalCount === 1 ? "task" : "tasks"} completed
              </p>
            </div>
          );
        })()}
          ...
          <div className="mb-8">
            ...
            >
              ...
              {primaryTask.description && (
                <p className="text-zinc-400 mb-4 whitespace-pre-line text-sm">
                  {primaryTask.description}
                </p>
              )}
                ...
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {task.description && (
                        <p className="text-xs text-zinc-400 whitespace-pre-line mt-2">
                          {task.description}
                        </p>
                      )}
                    </div>
                        ...
                        <span
                          className={
                            task.status === "complete"
                              ...
                              : "text-zinc-600 hover:text-green-500 text-sm"
                          }
                        >
                          ✓
                        </span>
                      ...
                      <span
                        className={
                          task.status === "complete"
                            ...
                            : "text-zinc-600 hover:text-green-500 text-xs"
                        }
                      >
                        ✓
                      </span>
        ...
        <div className="mt-12 pt-8 border-t border-zinc-800">
          <div className="text-center">
            <p className="text-zinc-500 mb-4">
              Done for today after completing above
            </p>
            <Link
              href="/wrap"
