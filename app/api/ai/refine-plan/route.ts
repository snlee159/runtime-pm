...
export async function POST(request: NextRequest) {
  try {
    ...
    // Format current plan
    const currentPlanText = formatCurrentPlan(currentPlan, availableTasks, lockedTaskIds)
      ...
      for (const lockedId of lockedTaskIds) {
        if (!allPlanTaskIds.includes(lockedId)) {
          ...
          if (currentPlan.primaryTaskId === lockedId) {
            ...
            result.secondary_task_ids = [...(result.secondary_task_ids || []), lockedId]
          } else if (currentPlan.multitaskTaskIds.includes(lockedId)) {
            result.multitask_task_ids = [...(result.multitask_task_ids || []), lockedId]
          }
        }
      }
    }
...
function formatCurrentPlan(currentPlan: any, allTasks: any[], lockedTaskIds: string[]) {
  const lines: string[] = []
  
  if (currentPlan.primaryTaskId) {
    const task = allTasks.find(t => t.id === currentPlan.primaryTaskId)
    if (task) {
      const locked = lockedTaskIds.includes(task.id) ? ' [LOCKED - COMPLETED]' : ''
      lines.push(`Primary Focus: ${task.title} (${task.estimated_effort}m, ${task.energy_cost} energy)${locked} [ID: ${task.id}]`)
    }
  } else {
  ...
  if (currentPlan.secondaryTaskIds.length > 0) {
    ...
    currentPlan.secondaryTaskIds.forEach((id: string) => {
      ...
      if (task) {
        const locked = lockedTaskIds.includes(task.id) ? ' [LOCKED - COMPLETED]' : ''
        lines.push(`- ${task.title} (${task.estimated_effort}m, ${task.energy_cost} energy)${locked} [ID: ${task.id}]`)
      }
    })
  } else {
  ...
  if (currentPlan.multitaskTaskIds.length > 0) {
    ...
    currentPlan.multitaskTaskIds.forEach((id: string) => {
      ...
      if (task) {
        const locked = lockedTaskIds.includes(task.id) ? ' [LOCKED - COMPLETED]' : ''
        lines.push(`- ${task.title} (${task.estimated_effort}m)${locked} [ID: ${task.id}]`)
      }
    })
  }
...
function formatAvailableTasks(allTasks: any[], currentPlan: any, lockedTaskIds: string[]) {
  ...
  byProject.forEach((tasks, projectName) => {
    lines.push(`\n${projectName}:`)
    tasks.forEach(task => {
      const deps = task.dependencies?.length > 0 ? ` [⚠️ ${task.dependencies.length} dependencies]` : ''
      lines.push(`- ${task.title} (${task.estimated_effort}m, ${task.energy_cost} energy, ${task.focus_depth} focus)${deps} [ID: ${task.id}]`)
    })
  })
